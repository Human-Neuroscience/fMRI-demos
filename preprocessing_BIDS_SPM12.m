...
It also assumes you acquired a T1 anatomical image, at least 1 functional run saved as a 4D nifti file, and a field map.
 
In order to run this script, call the preprocessing_BIDS_SPM12 function, giving it one subject numer.
The script will then run the preprocessing for that subejct.

23/01/2018 v.0.9.0: - stable version from David Wisniewski (david.wisniewski@ugent.be)
01/07/2018 v.1.0.0: - adapted to multi-band sequence
                    - added segmentation step
10/01/20222 v.1.0.1: - cleaned up a bit

Carlos Gonzalez (cgonzalez@ugr.es)
    %}
    %% FLAGS
    %{
...
in the order: deface -> fieldmap -> realign and unwarp -> slice time correction -> coregistration -> normalization -> smoothing
    ...
    basedir = 'D:\UGentData\2020_InstrAct_fMRI\fMRI\BIDS'; # adapt to corresponding directory in your system!
    ...
    derivativesdir = 'D:\UGentData\2020_InstrAct_fMRI\fMRI\derivatives'; # adapt to corresponding directory in your system!
    subdir=['\sub-' substr];
    mkdir([regexprep([basedir subdir],'BIDS','derivatives') '\anat']) # this nomenclature ('anat','fmap','func') complies with BIDS. Please, keep it this way if possible!
    mkdir([regexprep([basedir subdir],'BIDS','derivatives') '\fmap'])
    mkdir([regexprep([basedir subdir],'BIDS','derivatives') '\func'])
    ...
    if do_fieldmap
        matlabbatch{1}.spm.tools.fieldmap.calculatevdm.subj.data.presubphasemag.phase =  cellstr(spm_select('FPList',[basedir subdir '\fmap\'],'^sub.*phasediff.nii$'));
        % of the two magnitude images, use the one with the shorter TE,which will have greater signal
        matlabbatch{1}.spm.tools.fieldmap.calculatevdm.subj.data.presubphasemag.magnitude = cellstr(spm_select('FPList',[basedir subdir '\fmap\'],'^sub.*magnitude1.nii$'));
        ...
        matlabbatch{1}.spm.tools.fieldmap.calculatevdm.subj.defaults.defaultsval.mflags.template = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\toolbox\FieldMap\T1.nii'}; # adapt to corresponding directory in your system!
    ...
    if do_realign_unwarp
        ...
        vdmfile = cellstr(spm_select('FPList',[derivativesdir subdir '\fmap\'],'^vdm.*\phasediff.nii$'));
        for run=1:nruns
    ...
    if do_segment == 1
        ...
        matlabbatch{1}.spm.spatial.preproc.tissue(1).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,1'}; # adapt to corresponding directory in your system!
        matlabbatch{1}.spm.spatial.preproc.tissue(1).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(1).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(1).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,2'}; # adapt to corresponding directory in your system!
        matlabbatch{1}.spm.spatial.preproc.tissue(2).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(2).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(3).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,3'}; # adapt to corresponding directory in your system!
        matlabbatch{1}.spm.spatial.preproc.tissue(3).ngaus = 2;
        matlabbatch{1}.spm.spatial.preproc.tissue(3).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(3).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(4).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,4'}; # adapt to corresponding directory in your system!
        matlabbatch{1}.spm.spatial.preproc.tissue(4).ngaus = 3;
        matlabbatch{1}.spm.spatial.preproc.tissue(4).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(4).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(5).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,5'}; # adapt to corresponding directory in your system!
        matlabbatch{1}.spm.spatial.preproc.tissue(5).ngaus = 4;
        matlabbatch{1}.spm.spatial.preproc.tissue(5).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(5).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(6).tpm = {'C:\Users\carlos\Documents\MATLAB\spm12\spm12\tpm\TPM.nii,6'}; # adapt to corresponding directory in your system!
